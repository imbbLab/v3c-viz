var STYLES: any, namedEntities: any;
...
function randomString(holder: any) {
...
function createNamedToNumberedLookup(items: any, radix: any) {
    var i, entity, lookup: any = {}, base10, base16;
    items = items.split(',');
    radix = radix || 10;
...
function getTextAnchor(textAlign: any) {
    //TODO: support rtl languages
    var mapping: any = { "left": "start", "right": "end", "center": "middle", "start": "start", "end": "end" };
    return mapping[textAlign] || mapping.start;
}

//helper function to map canvas-textBaseline to svg-dominantBaseline
function getDominantBaseline(textBaseline: any) {
...
export class CanvasGradient {
     ...
     * @constructor
     */
    constructor(gradientNode: HTMLElement, ctx: SVGContext) {
     ...
     */
    addColorStop(offset: any, color: any) {
        var stop = this.__ctx.__createElement("stop"), regex, matches;
        stop.setAttribute("offset", offset);
        if (color && color.indexOf("rgba") !== -1) {
            //separate alpha value, since webkit can't handle it
            regex = /rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d?\.?\d*)\s*\)/gi;
            matches = <RegExpExecArray>regex.exec(color);
 ...
 */
export class SVGContext implements CanvasRenderingContext2D {
     ...
     * @private
     */
    private __applyStyleState(styleState: any) {
        let that = (<any>this);

        var keys = Object.keys(styleState), i, key;
        for (i = 0; i < keys.length; i++) {
            key = keys[i];
            //this.styles[key] = styleState[key];

            that[key] = styleState[key];
        }
    };
    ...
    private __getStyleState() {
        let that = (<any>this);
        var i, styleState: any = {}, keys = Object.keys(STYLES), key;
        for (i = 0; i < keys.length; i++) {
            key = keys[i];
            styleState[key] = that[key];
        }
        return styleState;
    };
     ...
     * @private
     */
    private __applyStyleToCurrentElement(type: any) {
        var currentElement = this.__currentElement;
        var currentStyleGroup = this.__currentElementsToStyle;
        if (currentStyleGroup) {
            currentElement.setAttribute(type, "");
            currentElement = currentStyleGroup.element;
            currentStyleGroup.children.forEach(function (node: any) {
            ...
            if (style.apply) {
                ...
                if (value instanceof SVGCanvasPattern) {
                    ...
                    if ((style.svgAttr === "stroke" || style.svgAttr === "fill") && value && value.indexOf("rgba") !== -1) {
                        ...
                        regex = /rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d?\.?\d*)\s*\)/gi;
                        matches = <RegExpExecArray>regex.exec(value);
     ...
     * @private
     */
    private __closestGroupOrSvg(node?: any): any {
    ...
    arcTo(x1: number, y1: number, x2: number, y2: number, radius: number) {
        ...
        var getAngle = function (vector: any) {
    ...
    __clearCanvas() {
        ...
        this.__currentElement = <Element>rootGroup;
   ...
   createRadialGradient(x0: number, y0: number, r0: number, x1: number, y1: number, r1: number) {
    ...
    __parseFont() {
        ...
        var fontPart = <RegExpExecArray>regex.exec(this.font);
        var data = {
   ...
   __wrapTextLink(font:any, element: Element) {
     ...
     * @private
     */
    __applyText(text: string, x: number, y: number, action: any) {
    ...
    drawImage(image:any, sx1:any, sy1:any, sWidth?:any, sHeight?:any, dx?:any, dy?:any, dWidth?:any, dHeight?:any) {
        ...
        if (image instanceof SVGContext) {
            ...
            if (svg.childNodes && svg.childNodes.length > 1) {
                ...
                while (defs.childNodes.length) {
                    let childNode = <Element>defs.childNodes[0]
                    id = childNode.getAttribute("id");
                    this.__ids[<string>id] = id;
    ...
    createPattern(image: CanvasImageSource, repetition: string | null): CanvasPattern | null{
        ...
        if (image instanceof HTMLElement && (image.nodeName === "CANVAS" || image.nodeName === "IMG")) {
            img = this.__createElement("image");
            img.setAttribute("width", image.width);
            img.setAttribute("height", image.height);
            img.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href",
                image.nodeName === "CANVAS" ? (<HTMLCanvasElement>image).toDataURL() : image.getAttribute("src"));
            pattern.appendChild(img);
            this.__defs.appendChild(pattern);
        } else if (image instanceof SVGContext) {
